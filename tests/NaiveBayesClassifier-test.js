...
describe('classifier correctness', function () {
  ...
  it('should categorizes correctly for `chinese` and `japanese` categories', function (done) {
    ...
    assert.equal(classifier.dataStore.getTokenFrequencyCount('chinese', '_chinese'), 5);
    assert.equal(classifier.dataStore.getTokenFrequencyCount('chinese', '_beijing'), 1);
    assert.equal(classifier.dataStore.getTokenFrequencyCount('chinese', '_shanghai'), 1);
    assert.equal(classifier.dataStore.getTokenFrequencyCount('chinese', '_macao'), 1);

    //make sure it learned the `japanese` category correctly
    assert.equal( classifier.dataStore.getTokenFrequencyCount('japanese', '_tokyo'), 1);
    assert.equal( classifier.dataStore.getTokenFrequencyCount('japanese', '_japan'), 1);
    assert.equal( classifier.dataStore.getTokenFrequencyCount('japanese', '_chinese'), 1);
